 ...
 * Static events data.
 *
 * Replace this file with calls to your own backend when you build an admin.
 * The shape below is what the frontend expects — keep it stable.
 */
...
const EVENTS_BY_LOCALE: Record<string, SchoolEvent[]> = {
  en: [
    {
      slug: 'open-day-nov',
...
export function getAllEvents(locale: string): SchoolEvent[] {
  return EVENTS_BY_LOCALE[locale] || EVENTS_BY_LOCALE.en
}

export function getEventBySlug(locale: string, slug: string): SchoolEvent | undefined {
  return getAllEvents(locale).find((e) => e.slug === slug)
}
